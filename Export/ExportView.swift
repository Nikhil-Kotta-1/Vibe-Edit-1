...
enum ExportMode: String, CaseIterable, Identifiable {
    ...
    case vibeEditProject = "VibeEdit Project (.vibeedit)"
...
struct ExportView: View {
    ...
    @State private var vibeEditResult: String?
    @State private var vibeEditSummary: (collect: Int, missing: Int, bytes: Int64) = (0, 0, 0)
    ...
    var body: some View {
        ...
        .task {
            loadPreview()
            vibeEditSummary = computeVibeEditSummary()
        }
    }
    ...
    private var settingsPanel: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                case .vibeEditProject:
                    VStack(alignment: .leading, spacing: AppTheme.Spacing.xs) {
                        Text("Saves a copy of this project with all media bundled inside, so it opens on any machine.")
                            .font(.system(size: AppTheme.FontSize.sm))
                            .foregroundStyle(AppTheme.Text.secondaryColor)

                        if vibeEditSummary.missing > 0 {
                            Text("\(vibeEditSummary.missing) media file\(vibeEditSummary.missing == 1 ? "" : "s") missing — they'll be skipped.")
                                .font(.system(size: AppTheme.FontSize.xs))
            ...
            HStack(spacing: AppTheme.Spacing.lg) {
                ...
                case .vibeEditProject:
                    HStack(spacing: AppTheme.Spacing.xs) {
                        ...
                        Text("~\(ByteCountFormatter.string(fromByteCount: vibeEditSummary.bytes, countStyle: .file))")
                    }
                }
            }
            .font(.system(size: AppTheme.FontSize.xs))
    ...
    private var exportFormat: ExportFormat {
        switch mode {
        case .xml, .vibeEditProject: .xml   // vibeEditProject has its own path; never rendered
        case .video:
            switch codec {
            case .h264: .h264
            case .h265: .h265
    ...
    /// Quick estimate for exporting a VibeEdit Project
    private func computeVibeEditSummary() -> (collect: Int, missing: Int, bytes: Int64) {
    ...
    private func startExport() {
        if mode == .vibeEditProject { startVibeEditExport(); return }
    ...
    private func startVibeEditExport() {
        vibeEditResult = nil
        let panel = NSSavePanel()
        ...
        panel.begin { response in
            guard response == .OK, let url = panel.url else { return }
            Task {
                let report = await service.exportVibeEditProject(
                    timeline: editor.timeline,
                ...
                if report.missing.isEmpty {
                    ...
                    vibeEditResult = "Exported, but \(report.missing.count) media file\(report.missing.count == 1 ? "" : "s") were missing and couldn't be included."
                }
            }
        }
    }
