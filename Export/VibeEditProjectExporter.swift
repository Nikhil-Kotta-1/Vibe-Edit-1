...
/// Writes a self-contained `.vibeedit` package: every resolvable media reference is copied
/// into the new bundle's `media/` directory and rewritten to a project-relative source
enum VibeEditProjectExporter {
    ...
    static func export(
        ...
        let staging = fm.temporaryDirectory.appendingPathComponent("vibeedit-export-\(UUID().uuidString)", isDirectory: true)
