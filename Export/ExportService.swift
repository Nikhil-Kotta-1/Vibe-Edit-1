...
final class ExportService {
    ...
    /// Writes a self-contained `.vibeedit` bundle (all media collected internally).
    @discardableResult
    func exportVibeEditProject(
        timeline: Timeline,
        manifest: MediaManifest,
        generationLog: GenerationLog,
        sourceProjectURL: URL?,
        outputURL: URL
    ) async -> VibeEditProjectExporter.Report? {
        isExporting = true
        progress = 0
        error = nil
        ...
        do {
            Log.export.notice("vibeedit export start url=\(outputURL.lastPathComponent)")
            let report = try await Task.detached(priority: .userInitiated) {
                try VibeEditProjectExporter.export(
            ...
            progress = 1.0
            Log.export.notice("vibeedit export ok collected=\(report.collected.count) missing=\(report.missing.count)")
            return report
        } catch {
            self.error = Log.detail(error)
            Log.export.error("vibeedit export failed: \(Log.detail(error))")
            return nil
        }
    }
