...
final class AgentService {
    ...
    private func selectClient() -> (any AgentClient)? {
        ...
        if AccountService.shared.isSignedIn {
            return VibeEditClient(model: chosen)
        }
        return nil
    }
    ...
    var streamError: VibeEditClientError?
    ...
    /// Runs one agent turn in the shared `currentTask` slot and returns its handle so
    /// callers (e.g. voice) can await completion. Cancels any in-flight turn first, so
    /// text and voice never run two `runLoop()`s against `messages` at once.
    @discardableResult
    func runAgentTurn() -> Task<Void, Never> {
        currentTask?.cancel()
        isStreaming = true
    ...
    private func runLoop() async {
        ...
        loop: while !Task.isCancelled {
            ...
            do {
                ...
                break loop
            } catch let err as VibeEditClientError {
                dropEmptyAssistantTurn(id: assistantID)
                streamError = err
                break loop
            } catch {
