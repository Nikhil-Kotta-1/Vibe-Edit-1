...
enum ToolName: String, CaseIterable, Sendable {
    ...
    case searchMediaMemory = "search_media_memory"
...
enum ToolDefinitions {
    static let all: [AgentTool] = [
        ...
        AgentTool(
            name: .searchMediaMemory,
            description: "Search the user's ENTIRE lifetime footage library — every shoot they've ever ingested, NOT just what's in the open project. Use this when the user refers to footage that isn't already in the project: \"a clip of my friend\", \"b-roll from my old shoots\", \"that video from last summer\", \"find footage of X from my library\". (For media already imported into the current project, use search_media instead.)\n\nMatching is semantic — phrase query like an image caption ('a person outdoors at sunset'), not keywords. Returns clips that are ALREADY CUT to length; place each one whole.\n\nBy default (import=true) each returned clip is imported into the project automatically and comes back with a ready-to-use mediaRef — pass it straight to add_clips (durationFrames = round(duration * fps), fps from get_timeline). Set import=false to preview matches without importing. duration/t_start/t_end are in SECONDS.\n\nRequires the local footage-memory service to be running; if it isn't, the tool says so — relay that to the user rather than concluding the footage doesn't exist.",
            inputSchema: objectSchema(
                properties: [
                    "query": ["type": "string", "description": "A caption-style description of the footage to find (e.g. 'a person speaking to camera indoors')."],
                    "has_speech": ["type": "boolean", "description": "Optional. true = only clips with spoken audio; false = silent b-roll only. Omit for both."],
                    "after": ["type": "string", "description": "Optional. Only clips created on/after this date, 'YYYY-MM-DD' (day-inclusive)."],
                    "before": ["type": "string", "description": "Optional. Only clips created on/before this date, 'YYYY-MM-DD' (day-inclusive)."],
                    "near_gps": [
                        "type": "array",
                        "description": "Optional. [latitude, longitude] to bias toward clips shot near a location.",
                        "items": ["type": "number"],
                    ],
                    "limit": ["type": "integer", "description": "Optional. Max clips to return (default 8, max 50). Also caps how many are auto-imported."],
                    "import": ["type": "boolean", "description": "Optional. Default true: import each match into the project and return its mediaRef. false: preview only (no mediaRef, nothing added to the library)."],
                ],
                required: ["query"]
            )
        ),
                ...
                properties: [
                    "source": [
                        ...
                        "properties": [
                            ...
                            "path": ["type": "string", "description": "Absolute local file or directory path, readable by the VibeEdit process. A directory is imported recursively — every openable file is pulled in and the folder structure is replicated as media folders."],
