...
extension ToolExecutor {
    ...
    private func importFromPath(editor: EditorViewModel, path: String, name: String?, folderId: String?) throws -> ToolResult {
        ...
        let asset = try importLocalFile(editor: editor, fileURL: fileURL, name: name, folderId: folderId)
        return .ok("Imported '\(asset.name)' (id: \(asset.id), type: \(asset.type.rawValue)) from path. Available now in get_media.")
    }

    /// Imports a single local media file into the project library and returns the asset.
    /// Shared by import_media (path source) and search_media_memory (auto-import).
    func importLocalFile(editor: EditorViewModel, fileURL: URL, name: String?, folderId: String?) throws -> MediaAsset {
        guard FileManager.default.fileExists(atPath: fileURL.path) else {
            throw ToolError("File not found: \(fileURL.path)")
        }
        ...
        guard let asset = editor.addMediaAsset(from: fileURL) else {
            throw ToolError("Failed to import file: \(fileURL.path)")
        }
