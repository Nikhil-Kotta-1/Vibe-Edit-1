...
struct VibeEditClient: AgentClient {
    ...
    private func run(
        ...
        guard let baseURL = BackendConfig.convexHttpURL else {
            throw VibeEditClientError.upstream("Backend not configured")
        }
        ...
        guard let session = await Clerk.shared.session, session.status == .active else {
            throw VibeEditClientError.unauthenticated
        }
        ...
        if let http = response as? HTTPURLResponse, http.statusCode >= 400 {
            ...
            throw VibeEditClientError.from(status: http.statusCode, body: body)
        }
...
enum VibeEditClientError: LocalizedError {
    ...
    static func from(status: Int, body: String) -> VibeEditClientError {
