...
struct MediaTab: View {
    ...
    private var overflowMenu: some View {
        ...
        return toolbarMenuIcon(systemName: "ellipsis") {
            ...
            Button(action: giveAccess) {
                Label("Give Access…", systemImage: "folder.badge.gearshape")
            }
            if canOrganize {
    ...
    /// Let the user pick folders to index into media-memory, so the agent can search them.
    private func giveAccess() {
        guard !MediaMemoryIngestService.shared.isRunning else {
            MediaMemoryConsoleWindowController.shared.show()
            return
        }
        let panel = NSOpenPanel()
        panel.canChooseDirectories = true
        ...
        panel.allowsMultipleSelection = true
        panel.message = "Choose folders to give the agent access to"
        panel.prompt = "Give Access"
        panel.begin { response in
            guard response == .OK, !panel.urls.isEmpty else { return }
            MediaMemoryIngestService.shared.start(folders: panel.urls)
            MediaMemoryConsoleWindowController.shared.show()
        }
    }
}
