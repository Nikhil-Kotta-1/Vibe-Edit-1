...
extension MediaTab {
    ...
    /// Media-memory ingest ("Give Access") status. Tapping opens the console window.
    @ViewBuilder
    var ingestStatusTag: some View {
        let ingest = MediaMemoryIngestService.shared
        switch ingest.state {
        case .idle:
            EmptyView()
        case .running:
            Button { MediaMemoryConsoleWindowController.shared.show() } label: {
            ...
            .buttonStyle(.plain)
            .help("Indexing folders into media memory. Click to open the console.")
        case .finished:
            ingestStatusButton(icon: "checkmark.circle", label: "Finished",
                               tint: Color(AppTheme.TrackColor.audio),
                               help: "Media memory is up to date. Click to open the console.")
        case .failed:
            ingestStatusButton(icon: "exclamationmark.triangle", label: "Failed",
                               tint: AppTheme.Status.errorColor,
                               help: "Indexing failed. Click to open the console.")
        }
    }

    private func ingestStatusButton(icon: String, label: String, tint: Color, help: String) -> some View {
        Button { MediaMemoryConsoleWindowController.shared.show() } label: {
