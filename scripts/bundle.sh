...
SIGNING_IDENTITY="${SIGNING_IDENTITY:-Developer ID Application: VibeEdit, Inc. (MMFLRC7562)}"
NOTARY_PROFILE="${NOTARY_PROFILE:-vibeedit-notary}"
SENTRY_DSN="${SENTRY_DSN:-}"
RESOURCES="$ROOT/Sources/VibeEdit/Resources"
APP="$ROOT/.build/VibeEdit.app"
ZIP="$ROOT/.build/VibeEdit.zip"
DMG="$ROOT/.build/VibeEdit.dmg"

echo "==> Building ($CONFIG)"
swift build -c "$CONFIG"
BIN="$(swift build -c "$CONFIG" --show-bin-path)/VibeEdit"
SPARKLE_FW="$ROOT/.build/artifacts/sparkle/Sparkle/Sparkle.xcframework/macos-arm64_x86_64/Sparkle.framework"

echo "==> Assembling $APP"
rm -rf "$APP"
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources" "$APP/Contents/Frameworks"
cp "$BIN" "$APP/Contents/MacOS/VibeEdit"
...
inject_plist VibeEditClerkPublishableKey "${CLERK_PUBLISHABLE_KEY:-}"
inject_plist VibeEditConvexDeploymentURL "${CONVEX_DEPLOYMENT_URL:-}"
inject_plist VibeEditConvexHttpURL "${CONVEX_HTTP_URL:-}"
cp "$RESOURCES/AppIcon.icns" "$APP/Contents/Resources/AppIcon.icns"
cp -R "$SPARKLE_FW" "$APP/Contents/Frameworks/Sparkle.framework"

# Flatten SwiftPM's resource bundle into the app's Resources tree.
RES_BUNDLE="$(dirname "$BIN")/VibeEdit_VibeEdit.bundle"
...
if [ -f "$RES_BUNDLE/vibeedit.mcpb" ]; then
  cp "$RES_BUNDLE/vibeedit.mcpb" "$APP/Contents/Resources/"
else
  echo "!! missing vibeedit.mcpb in SwiftPM resource bundle at $RES_BUNDLE" >&2
  exit 1
fi
...
# Bundle the other dynamic frameworks SwiftPM linked via @rpath (LiveKit's
# LiveKitWebRTC + RustLiveKitUniFFI, pulled in by the voice/ElevenLabs SDK).
BIN_DIR="$(dirname "$BIN")"
for dep in $(otool -L "$APP/Contents/MacOS/VibeEdit" | awk -F'[/ ]' '/@rpath\/.+\.framework\//{print $2}' | sort -u); do
  [ "$dep" = "Sparkle.framework" ] && continue
  [ -d "$APP/Contents/Frameworks/$dep" ] && continue
  if [ -d "$BIN_DIR/$dep" ]; then
    echo "==> Bundling $dep"
    cp -R "$BIN_DIR/$dep" "$APP/Contents/Frameworks/$dep"
  else
    echo "!! $dep linked via @rpath but missing from $BIN_DIR — app will crash at launch" >&2
  fi
done

install_name_tool -add_rpath "@executable_path/../Frameworks" "$APP/Contents/MacOS/VibeEdit"
touch "$APP"
...
DSYM="$ROOT/.build/VibeEdit.dSYM"
echo "==> Generating dSYM"
rm -rf "$DSYM"
dsymutil "$APP/Contents/MacOS/VibeEdit" -o "$DSYM"
