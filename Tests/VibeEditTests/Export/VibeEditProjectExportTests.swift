...
@Suite("VibeEdit project export (collect media)")
struct VibeEditProjectExportTests {
    ...
    private func makeFixture() throws -> (root: URL, source: URL, dest: URL, externalContents: String) {
        ...
        let source = root.appendingPathComponent("Source.vibeedit", isDirectory: true)
        ...
        return (root, source, root.appendingPathComponent("Packaged.vibeedit", isDirectory: true), externalContents)
    }
    ...
    @Test func collectsExternalAndInternalRewritesSourcesReportsMissing() throws {
        ...
        let report = try VibeEditProjectExporter.export(
