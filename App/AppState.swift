...
final class AppState {
    ...
    private(set) var voiceService: VoiceConversationService?

    /// Lazily creates the single app-wide voice service. It resolves the active editor
    /// on each tool call, so a session always targets the front project.
    @discardableResult
    func voice() -> VoiceConversationService {
        if let voiceService { return voiceService }
        let service = VoiceConversationService(editorProvider: { [weak self] in
